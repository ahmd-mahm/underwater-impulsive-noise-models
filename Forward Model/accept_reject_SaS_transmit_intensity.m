...
nbins = 100;
figure
[h,bins]=logloghistquant(Pr_abs,nbins,[10^-4,1-10^-5]);
fig_hist=gcf;
hold on
...
figure
[h_It,bins_It]=logloghistquant(It,nbins,[10^-6,1-10^-6]);
xlabel('I_t')
ylabel('pdf')

figure
logyhistquant(It,nbins,[10^-4,1-10^-4]);
xlabel('I_t')
ylabel('pdf')

figure
dBhistquant(It,nbins,[10^-6,1-10^-6]);
xlabel('I_t (dB)')
ylabel('pdf')

figure
dBhistquant(Ir,nbins,[10^-6,1-10^-6]);
xlabel('I_r (dB)')
ylabel('pdf')

figure
logloghistquant(Ir,nbins,[10^-6,1-10^-6]);
xlabel('I_r')
ylabel('pdf')
...
function [h,bins]=logloghistquant(x,nbins,L)
if nargin==3
...
function [h,bins]=logyhistquant(x,nbins,L)
if nargin==3
    qnt=quantile(x,L);
else
...
function [h,dB_bins]=dBhistquant(x,nbins,L)
x=10*log10(x);
if nargin==3
    qnt=quantile(x,L);
else
